...
export async function POST(request: Request) {
  try {
    ...
    if (!isCron) {
      ...
      // Estar logueado no alcanza: este endpoint escribe en `matches` con la
      // service_role, así que lo reservamos para cuentas ya aprobadas.
    ...
    return NextResponse.json(
      { error: 'Error interno', details: mensajeDeError(err) },
      { status: 500 }
    )
  }
}
